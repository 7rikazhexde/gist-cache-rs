...
**feat(install): シェル補完設定をインストールスクリプトに統合** ✅

- **問題**: インストールスクリプトでシェル補完設定の自動化がされていなかった。
- **修正**: `setup.ps1` と `setup.sh` にシェル補完設定ロジックを追加。
- **実装**: `script/setup.ps1`, `script/setup.sh` を修正。

**feat(cli): `config show` コマンドの出力改善** ✅

- **問題**: `gist-cache-rs config show` 実行時、設定が空の場合にユーザーへの案内が不足していた。
- **修正**: 設定が空の場合に、設定可能なオプションと設定方法を案内するメッセージを表示。
- **実装**: `src/cli.rs` を修正。
- **テスト**: `tests/cli_tests.rs` に統合テストを追加。

#### v0.8.6 (実装完了、リリース準備中)

**3.4 高度なインタープリター設定と優先度ベース解決** ✅

- **機能概要**: 拡張子ごとのインタープリター設定と優先度ベースの自動検出
- **実装内容**:
  - 拡張子ごとのデフォルトインタープリター設定（`defaults.interpreter.<extension>`）
  - ワイルドカードフォールバック（`defaults.interpreter."*"`）
  - 6段階の優先度システム:
    1. コマンドライン引数（最優先）
    2. Shebang検出（`#!/usr/bin/env python3`）
    3. ユーザー設定（拡張子ベース）
    4. ヒューリスティック（ファイル名+拡張子パターン）
    5. コンテンツベース言語検出（tokei使用）
    6. グローバルデフォルト（ワイルドカードまたはbash）
  - 自動Shebang検出機能
  - 特殊ファイル名検出（例：Makefile → make）
  - レガシー設定との後方互換性
- **実装**: `src/config.rs`、`src/cli.rs` を大幅修正（900行以上追加）
  - `InterpreterSetting` enum追加（Single/Multiple対応）
  - 検出ヘルパー関数群追加（shebang、拡張子、ファイル名、言語検出）
  - `resolve_interpreter()` 関数で優先度ベース解決を実装
- **テスト**: 148テスト全て成功（141 unit + 7 execution）
  - 3個のconfig tests（nested、legacy、persistence）
  - 7個のcli helper tests（shebang、extension、filename検出）
  - 9個の手動テストGist作成・検証
  - 既存機能のデグレなし確認済み
- **検証**: 9個の公開Gistで包括的なテスト
  - Python/Ruby/TypeScriptの引数あり/なし
  - Bash/Node.jsの引数処理
  - uv PEP 723メタデータとpandasインストール
- **ドキュメント**:
  - `CHANGELOG.md` 更新
  - `.claude/docs/feature_update_plan_config_and_run.md` 作成
  - `.claude/docs/interpreter_resolution_test_results.md` 作成（テストGist URL含む）
- **効果**:
  - 柔軟性: 異なるファイルタイプに異なるインタープリターを使用可能
  - 知的検出: Shebangとファイルコンテンツから自動判定
  - 互換性: レガシー単一インタープリター設定をサポート
  - ユーザー制御: 明確な優先順位でコマンドラインオーバーライド可能
  - 安全性: 複数の検出方法を通じて適切にフォールバック
...
4. ⏳ エラーメッセージを改善 (5.2.2) - 計画中
5. ⏳ パストラバーサル保護を追加 (5.3) - 計画中

**成果物**: バージョン 0.7.0 (完了), 0.8.0 (完了)

---

### フェーズ2：ユーザー体験（第3-4週） - 完了

**目標**: 使いやすさを向上させ、対話的なフィードバックを強化

1. ✅ プログレス表示 (2.2) - **完了 (v0.8.1)**
2. ✅ 対話的選択の改善 (2.3) - **完了 (v0.8.2)**
3. ✅ `config`サブコマンドのタブ補完修正 (1.3の改善) - **完了 (v0.8.4)**
4. ✅ シェル補完ドキュメントの改善 - **完了 (v0.8.4以降)**
5. ✅ インストールスクリプトへのシェル補完設定の統合 - **完了 (v0.8.5)**
6. ✅ `config show` の出力改善 (空の場合) - **完了 (v0.8.5)**
7. ✅ 高度なインタープリター設定と優先度ベース解決 (3.4) - **完了 (v0.8.6)**
8. 🔜 設定ファイルサポート (1.2) - 将来検討
9. 🔜 検索強化 - 正規表現と言語フィルタ (2.1) - 計画中
10. 🔜 出力フォーマットオプション (2.4) - 計画中
11. 🔜 実行確認 (5.1.1) - 計画中

**成果物**: バージョン 0.8.1 (完了), 0.8.2 (完了), 0.8.4 (完了), 0.8.5 (完了), 0.8.6 (完了)

---

### フェーズ3：機能拡張（第5-6週） - 計画中

**目標**: 新機能を追加し、ユースケースを拡大
...
### フェーズ4：高度な機能（第7-8週） - 計画中

**目標**: パワーユーザー向け機能を追加し、パフォーマンスを最適化
