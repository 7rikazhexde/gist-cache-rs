...
**ステータス**: ✅ 部分的に実装完了 (正規表現・言語・拡張子フィルタ) / タグベース検索は未実装
...
- ✅ 2.1.1 正規表現検索 (`--regex` フラグ)
  - パターンマッチングによる高度な検索が可能
  - 説明文とファイル名の両方で正規表現が使用可能
  - 無効な正規表現パターンはエラーを返す
- ✅ 2.1.2 言語/ファイルタイプフィルタ
  - `--language` フラグで言語による絞り込み (大文字小文字を区別しない)
  - `--extension` フラグで拡張子による絞り込み (`.`の有無は自動処理)
  - 複数ファイルを持つgistでは、いずれかのファイルがマッチすれば結果に含まれる
- ✅ フィルタの組み合わせが可能
  - `--regex`、`--language`、`--extension` を同時に使用可能
  - すべての条件を満たすgistのみが表示される
- ✅ 後方互換性を維持
  - 既存の `SearchQuery::new()` は変更なし
  - 新しい `SearchQuery::new_with_options()` を追加
- ✅ 包括的なテストカバレッジ
  - 15個の新しい単体テストを追加
  - 全146個のライブラリテストが成功
...
- ❌ 2.1.3 タグベース検索 (`--tag` フラグ)
  - GitHub gistのトピック/タグ情報の取得・保存が必要
  - 将来のリリースで実装予定
...
5. ✅ インストールスクリプトへのシェル補完設定の統合 - **完了 (v0.8.4以降)**
6. ✅ `config show` の出力改善 (空の場合) - **完了 (v0.8.4以降)**
7. 🔜 設定ファイルサポート (1.2) - 将来検討
8. 🔜 検索強化 - 正規表現と言語フィルタ (2.1) - 計画中
9. 🔜 出力フォーマットオプション (2.4) - 計画中
10. 🔜 実行確認 (5.1.1) - 計画中

**成果物**: バージョン 0.8.1 (完了), 0.8.2 (完了), 0.8.4 (完了)
